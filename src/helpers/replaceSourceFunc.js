...
export function getCssChunkLoadFuncVarName(source, funcOriginSourceBody) {
  let result = '';
  const [leftPart] = source.split(funcOriginSourceBody);
  const match = leftPart.match(/\[((\w|\d)+?)\] ?= ?new Promise.*?$/m);
