...
describe('UploadPlugin:option-exclude', () => {
  beforeEach(() => {
  ...
  test('should replace all returned url', done => {
    ...
    const webpackConfig = getWebpackConfig(
      new UploadPlugin({
        uploader,
        options: { muteLog: true, exclude: ['index.html', /\.ttf$/, /\.js$/, /^dynamicSub\..*bundle\.js$/, /dynamic\..*bundle\.css/] },
      }),
    );
  ...
  test('should replace all returned url', done => {
    ...
    const webpackConfig = getWebpackConfig(
      new UploadPlugin({
        uploader,
        options: { muteLog: true, exclude: ['index.html', /\.ttf$/, /\.css$/, /^dynamicSub\..*bundle\.js$/, /dynamic\..*bundle\.css/] },
      }),
    );
    const compiler = webpack(webpackConfig, function callback(error, result) {
      expect(error).toBeFalsy();
    ...
    const webpackConfig = getWebpackConfig(
      new UploadPlugin({
        ...
        options: {
          muteLog: true,
          gather: true,
          exclude: [/\.ttf$/, /\.js$/, /\.css$/, /^dynamicSub\..*bundle\.js$/, /^dynamic\..*bundle\.css/],
          include: ['index.html', /\.ttf$/, /\.js$/, /\.css$/, /^dynamicSub\..*bundle\.js$/, /^dynamic\..*bundle\.css$/],
        },
      }),
    );
    const compiler = webpack(webpackConfig, function callback(error, result) {
